...
(function(d){
    ...
    H5F.pattern = function(el, type) {
        ...
        } else if(!type || type === "password") { // Password hst't be evalutated.
            return false;
        } else {
...
$(document).ready(function() {
	...
	messagesRef = ref.child("data/hsData");
	...
	$("#Register").click(function (e){
		 if(authData){
			...
			var hst_address = $('#address').val();
			var hst_country = $('#country').val();
			var hst_postcode = $('#postcode').val();
			var hst_gender = $('#gender').val();
			var hst_password = $('#password').val();
			var hst_rollnum = $('#rollnum').val();
			...
			//push data to hstent DB
			messagesRef.push({first_name:first_name,Birth_Day:Birth_Day,Birth_Month:Birth_Month,Birth_Year:Birth_Year,father_name:father_name,email_id:email_id,mobile_no:mobile_no,hst_address:hst_address,hst_country:hst_country,hst_postcode:hst_postcode,hst_gender:hst_gender,hst_designation:designation,hst_ID:hst_rollnum});
			
			//push data to login db
			loginRef.push({email:email_id,role:"h"});
			...
			window.alert("hostel staff Added");
